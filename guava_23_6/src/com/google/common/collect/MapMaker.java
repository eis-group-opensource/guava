...
public final class MapMaker {
  ...
  /**
   *
   * @param computingFunction funtion for cache creation
   * @param <K> generic key type
   * @param <V> generic value type
   * @return {@link com.google.common.cache.LocalReloadingCache}
   * @deprecated
   * This method is to support Richfaces 4.3.x. This method should never be used in EIS suite
   */
  @Deprecated
  public <K, V> ConcurrentMap<K, V> makeComputingMap(Function<K, V> computingFunction) {
    CacheLoader<K,V> cacheLoader = CacheLoader.from(computingFunction);
    LoadingCache<K,V> loadingCache = cacheBuilder.build(cacheLoader);
    ConcurrentMap<K,V> reloadingCacheMap = loadingCache.asReloadingMap();
    return reloadingCacheMap;
  }

  /**
   *
   * @return MapMaker with weak values
   *
   * @since 23.6-jre-e1
   *
   * @deprecated
